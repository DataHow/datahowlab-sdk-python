...
class ProductValidator(AbstractValidator):
    ...
    def validate(self, entity: Product, client: Client) -> bool:
        ...
        if len(entity.code) > 6:
            raise ImportValidationException("Product code must be from 1 to 6 characters long")
...
class ExperimentFileValidator(AbstractFileValidator):
    ...
    def validate(
        ...
        for variable in variables:
            ...
            if not isinstance(variable_data, dict):
                validation_errors.append(
                    (f"Variable {variable.code} data must be a dictionary withthe mandatory fields {sample_id} and 'values'")
                )
                continue

            if sample_id not in variable_data or "values" not in variable_data:
                validation_errors.append(f"Variable {variable.code} data must contain {sample_id} and 'values' key")
                continue
            ...
            if not isinstance(ids, list) or not isinstance(values, list):
                validation_errors.append(f"Variable {variable.code} data must contain {sample_id} and 'values' as lists")
                continue

            if len(ids) != len(values) or len(ids) == 0:
                validation_errors.append(
                    (f"Variable {variable.code} data can't be empty and must have equal lengthof {sample_id} and values")
                )
                continue
